...
def _concat_img_one_region(args):
    ...
    if len(tiles) < 20000:
        ...
        if mask_whole.sum() != 0:
            ...
            # save the image
            im = im * filled_mask.reshape(filled_mask.shape[0], filled_mask.shape[1], 1)  # type: ignore
...
def _concatenate_tiles(
    ...
    args = [
        (
            ...
            area_shp.loc[area_shp.index == i, "Y_X"].item(),  # type: ignore
            {
                key: cached_tiles[key]
                for key in area_shp.loc[area_shp.index == i, "Y_X"].item()  # type: ignore
...
def download_sateimgs(
    ...
    # coordinate system
    area_shp = area_shp.to_crs(epsg=4326)  # type: ignore

    # get the x and y coordinates of the tiles
    area_shp, Y_X = get_YX_area(area_shp)  # type: ignore
